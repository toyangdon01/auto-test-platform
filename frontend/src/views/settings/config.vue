...
const defaultConfig = {
  ...
  ssh_timeout: 86400000,  // 24小时（毫秒）
  exec_timeout: 86400,   // 24小时（秒）
...
const fetchConfig = async () => {
  loading.value = true
  try {
    const res = await request.get<Record<string, string>>('/system/config')
...
const handleSave = async () => {
  ...
  try {
    ...
    const res = await request.put('/system/config', configMap)
    if (res.code === 0) {
