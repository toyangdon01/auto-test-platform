...
// 表格选择变化
function handleSelectionChange(selection: ServerWithState[]) {
  selectedIds.value = selection.map(item => item.id)
}

// 批量删除
async function handleBatchDelete() {
  await ElMessageBox.confirm(`确定要删除选中的 ${selectedIds.value.length} 台服务器吗？`, '批量删除', { type: 'warning' })
  const res = await request.delete('/servers/batch', { data: selectedIds.value })
  if (res.code === 0) {
...
async function handleBatchDisable() {
  await ElMessageBox.confirm(`确定要禁用选中的 ${selectedIds.value.length} 台服务器吗？`, '批量禁用', { type: 'warning' })
  const res = await request.post('/servers/batch/disable', selectedIds.value)
  if (res.code === 0) {
...
async function handleBatchEnable() {
  const res = await request.post('/servers/batch/enable', selectedIds.value)
  if (res.code === 0) {
