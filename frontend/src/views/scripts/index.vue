    ...
    <div class="page-header">
      ...
      <div class="header-actions">
        ...
        <el-button @click="handleBatchExport" :disabled="selectedScripts.length === 0">
          ...
          批量导出{{ selectedScripts.length > 0 ? `(${selectedScripts.length})` : '' }}
        </el-button>
...
import { Plus, List, Upload, Download } from '@element-plus/icons-vue'
...
// 导入导出相关方法
function handleSelectionChange(selection: Script[]) {
...
function handleBatchExport() {
  if (selectedScripts.value.length === 0) {
    ElMessage.warning('请选择要导出的脚本')
    return
  }
  const ids = selectedScripts.value.map(s => s.id)
  exportDialogRef.value?.open(ids)
}
</script>
