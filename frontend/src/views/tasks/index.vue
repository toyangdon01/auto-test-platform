  ...
  <div class="page-card">
    ...
    <el-table v-loading="loading" :data="tableData" stripe>
      ...
      <el-table-column label="操作" width="220" fixed="right">
        <template #default="{ row }">
          <template v-if="row.status === 'running'">
            <el-button type="success" link @click="showRealtimeLog(row)">
          ...
          <template v-else>
            ...
            <el-button type="success" link @click="handleExecute(row)">
              <el-icon><VideoPlay /></el-icon>{{ row.status === 'pending' ? '执行' : '再次执行' }}
            </el-button>
            <el-button v-if="row.status !== 'running'" type="danger" link @click="handleDelete(row)">
              <el-icon><Delete /></el-icon>删除
            </el-button>
            <el-button v-if="row.status !== 'pending'" type="primary" link @click="handleDetail(row)">详情</el-button>
          </template>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="realtimeLogVisible" title="实时执行日志" width="90%" top="3vh" :close-on-click-modal="false">
      <div class="realtime-log-container">
        <TaskLogStream v-if="realtimeLogVisible" :task-id="currentTaskId" task-status="running" />
      </div>
    </el-dialog>
  </div>
</template>
...
async function fetchScripts() {
  try {
    const res = await request.get('/scripts', { page: 1, pageSize: 1000 })
    if (res.code === 0) {
...
// 显示实时日志
function showRealtimeLog(row: any) {
