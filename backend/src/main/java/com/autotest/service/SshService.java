...
public class SshService {
     ...
     * 设置默认连接超时（毫秒）
     */
    public static void setDefaultTimeout(int timeoutMs) {
     ...
     * 设置默认执行超时（毫秒）
     */
    public static void setDefaultExecTimeout(int timeoutMs) {
        defaultExecTimeout = timeoutMs;
    }
    ...
    public static String executeCommand(Server server, String command) {
        ExecuteResult result = executeCommand(server, command, null, defaultExecTimeout);
    ...
    public static ExecuteResult executeCommandWithResult(Server server, String command) {
        return executeCommand(server, command, null, defaultExecTimeout);
    }
    ...
    /**
     * 执行远程命令（PTY 模式，适合实时更新的程序如 fio）
     * 
     * PTY 模式会启用伪终端，可以捕获使用 \r 实时更新的输出
     *
     * @param server      服务器信息
     * @param command     命令
     * @param outputCallback 实时输出回调（可为null）
     ...
     * @return 执行结果
     */
    public static ExecuteResult executeCommandPty(Server server, String command, Consumer<String> outputCallback, int timeoutMs) {
        Session session = null;
