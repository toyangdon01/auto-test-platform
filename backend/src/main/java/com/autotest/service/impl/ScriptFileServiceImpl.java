...
public class ScriptFileServiceImpl implements ScriptFileService {
    ...
    public Map<String, Object> uploadScriptFile(MultipartFile file) throws IOException {
        ...
        if (isArchiveFile(extension)) {
            ...
            Path extractDir = tempFilePath.getParent().resolve("extracted");
            List<Map<String, Object>> fileList = extractArchive(tempFilePath.toString(), scriptName);
            result.put("fileList", fileList);
            result.put("isArchive", true);
            
            // 返回解压后的目录路径，而不是 zip 文件路径
            result.put("tempPath", extractDir.toString());
            result.put("isExtracted", true);

            // 自动检测入口文件
            String entryFile = detectEntryFile(fileList);
            result.put("suggestedEntry", entryFile);
            
            // 检查并解析 autotest.yaml 配置文件
            Path configFile = extractDir.resolve("autotest.yaml");
            if (Files.exists(configFile)) {
                try {
                    ...
                    com.autotest.config.ScriptConfig config = yamlMapper.readValue(configFile.toFile(), com.autotest.config.ScriptConfig.class);
                    result.put("config", config);
                    log.info("解析 autotest.yaml 成功: {}", configFile);
                } catch (Exception e) {
                    log.warn("解析 autotest.yaml 失败: {}", e.getMessage());
    ...
    @Override
    public void deleteScriptFile(Long scriptId, String filePath) throws IOException {
        Path scriptPath = Paths.get(scriptsPath, scriptId.toString(), filePath);
        if (!Files.exists(scriptPath)) {
            throw new FileNotFoundException("文件不存在: " + filePath);
        }
        ...
        log.info("删除脚本文件: scriptId={}, file={}", scriptId, filePath);
        
        // 删除空的父目录
        Path scriptDir = Paths.get(scriptsPath, scriptId.toString());
        cleanupEmptyDirectories(scriptDir, scriptPath.getParent());
    }

    @Override
    public void clearScriptFiles(Long scriptId) throws IOException {
        Path scriptDir = Paths.get(scriptsPath, scriptId.toString());
        if (!Files.exists(scriptDir)) {
            return;
        }
        ...
        log.info("清空脚本目录: scriptId={}", scriptId);
    }

    /**
     * 清理空的父目录
     */
    private void cleanupEmptyDirectories(Path rootDir, Path dir) throws IOException {
