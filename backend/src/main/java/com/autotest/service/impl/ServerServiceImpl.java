...
public class ServerServiceImpl implements ServerService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void batchDeleteServers(java.util.List<Long> ids) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void batchUpdateEnabled(java.util.List<Long> ids, Boolean enabled) {
        if (ids == null || ids.isEmpty()) {
            throw BusinessException.of("请选择要操作的服务器");
        }
        for (Long id : ids) {
            Server server = new Server();
            server.setId(id);
