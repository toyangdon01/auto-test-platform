...
public interface TaskStepMapper extends BaseMapper<TaskStep> {
    ...
    @Select("SELECT ts.*, s.name as server_name, s.host as server_host " +
            "FROM task_steps ts " +
            "LEFT JOIN servers s ON ts.server_id = s.id " +
            "WHERE ts.task_id = #{taskId} AND ts.step_name = #{stepName} AND ts.server_id = #{serverId}")
    ...
    List<TaskStep> findByTaskAndStepName(@Param("taskId") Long taskId, @Param("stepName") String stepName);
