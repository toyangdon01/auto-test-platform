...
public class Task implements Serializable {
    ...
    /**
     * 步骤服务器映射（JSONB）
     * 格式: {stepName: [serverId1, serverId2]}
     * 指定每个步骤在哪些服务器上执行
    ...
    /**
     * 获取步骤服务器映射（类型安全版本）
     * 将 JSON 反序列化的 Integer 转换为 Long
     */
    @SuppressWarnings("unchecked")
    public Map<String, List<Long>> getStepServerMappingAsLong() {
        if (stepServerMapping == null) return null;
        Map<String, List<Long>> result = new java.util.HashMap<>();
        for (Map.Entry<String, List<Object>> entry : stepServerMapping.entrySet()) {
            ...
            if (entry.getValue() != null) {
                for (Object obj : entry.getValue()) {
                    if (obj instanceof Long) {
                        ...
                        longList.add(((Integer) obj).longValue());
                    } else if (obj instanceof Number) {
                        longList.add(((Number) obj).longValue());
                    }
                }
            }
            result.put(entry.getKey(), longList);
        }
        return result;
    }
}
