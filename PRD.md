...
> 更新日期：2026-03-23  
> 文档状态：初稿  
> 更新记录：功能精简（删除结果对比/趋势分析）、创建任务简化、WebShell终端、多角色测试
...
| 多角色测试 | 支持同一服务器多角色（server/client）定义与执行 |
| WebShell 终端 | 基于 WebSocket 的服务器远程终端 |
| 性能指标实时采集 | CPU/内存/磁盘/网络指标采集，支持自定义指标 |
| 测试任务调度执行 | 立即执行，串行/并行（内部默认） |
| 测试报告自动生成 | PDF/HTML 格式导出 |
...
| **WebShell 终端** | P1 | 基于 WebSocket 的浏览器终端，支持实时交互 |
...
| **多角色配置** | P1 | 支持定义多角色（server/client），各角色独立入口函数和参数 |
| 脚本编辑 | P1 | 在线编辑脚本内容 |
...
| 功能点 | 优先级 | 描述 |
|--------|--------|------|
| 创建测试任务 | P0 | 配置服务器、脚本、参数、生命周期、指标采集（默认禁用） |
| 任务编辑 | P1 | 编辑未开始的任务 |
| 任务复制 | P1 | 快速创建相似任务 |
| 立即执行 | P0 | 任务创建后立即执行（默认） |
...
| 指标采集配置 | JSON | ❌ | 性能指标采集配置（默认禁用） |
| 执行方式 | Enum | 自动 | 固定为立即执行 |
| 执行策略 | Enum | 自动 | 固定为串行 |
| 失败策略 | Enum | 自动 | 固定为继续执行 |
...
| 功能点 | 优先级 | 描述 |
|--------|--------|------|
| 结果解析 | P0 | 根据脚本解析规则解析测试输出（仅 resultParser=true 的步骤） |
...
> **注**：结果对比分析和趋势分析功能已在 v1.0.3 版本中移除，如需此功能请考虑后续版本。
